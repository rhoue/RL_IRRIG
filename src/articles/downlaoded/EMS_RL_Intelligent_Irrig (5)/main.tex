...
%% Abstract
\begin{abstract}
Decision-making in irrigation systems often relies on simplified physical models that are interpretable but imperfect under variable climatic conditions. Reinforcement learning offers adaptive control capabilities, yet its performance can be sensitive to model mismatch and delayed system responses. This study presents a control-aware benchmarking framework comparing three irrigation strategies within a unified simulation environment under identical stochastic weather forcing: rule-based control, reinforcement learning applied to a fixed physics-based soil water model, and a hybrid approach combining reinforcement learning with a learned residual correction. The experimental setup distinguishes latent physical states from sensor-level observations and introduces a discrete-time residual inspired by Neural ODE concepts. Results from seasonal simulations show that reinforcement learning reduces irrigation volumes relative to rule-based control but may induce larger soil water tension excursions. Incorporating residual corrections moderates extreme tension behaviour while preserving most water-use efficiency gains.
\end{abstract}

"""
Module pour charger des données réelles (CSV, API, ERA5-Land) pour l'irrigation intelligente.
...
- Données ERA5-Land (optionnel, feature-flag)
"""
...
def _resolve_file_path(file_path: str) -> str:
    """
    Resolve user-provided file paths robustly across launch directories.
    """
    ...
    # Also support paths relative to the repository root when app is launched from src/.
...
def load_era5_land_dataset(
    ...
    """
    Charge un bundle ERA5-Land et le convertit en dictionnaire numpy.

    Args:
        file_path: Chemin du fichier NetCDF/Zarr ERA5-Land.
        target_grid: Grille cible {"lat": array, "lon": array} pour interpolation.
        soil_depth_mapping: Mapping des couches de sol vers indices ERA5-Land.
        resample_freq: Fréquence de rééchantillonnage temporel (ex: "1D").
        lat_name, lon_name, time_name: Noms des dimensions.
        drop_vars: Variables à ignorer pour alléger la mémoire.
    """
    from src.data_sources.era5_land import (
        build_era5_land_bundle,
        load_era5_land_raw,
    )

    ds = load_era5_land_raw(file_path, drop_vars=drop_vars, engine=engine)
    return build_era5_land_bundle(
        ds,
        target_grid=target_grid,
        soil_depth_mapping=soil_depth_mapping,
        lat_name=lat_name,
        lon_name=lon_name,
        time_name=time_name,
        freq=resample_freq,
...
def load_data_for_simulation(
    ...
    **kwargs
    ...
    if data_source == "csv":
        ...
        # Chargement météo
        rain, et0, Kc = load_weather_from_csv(resolved_file_path, **kwargs)
        
        # Chargement tensions si fichier séparé
        tension_file = kwargs.get("tension_file")
        tension = None
        if tension_file:
            tension = load_tension_from_csv(_resolve_file_path(tension_file), **kwargs)
        ...
        if not file_path:
            raise ValueError("file_path requis pour data_source='era5_land'")
        era5_land_cfg = {
            "target_grid": kwargs.get("target_grid"),
            "soil_depth_mapping": kwargs.get("soil_depth_mapping"),
            "resample_freq": kwargs.get("resample_freq", "1D"),
            "lat_name": kwargs.get("lat_name", "latitude"),
            "lon_name": kwargs.get("lon_name", "longitude"),
            "time_name": kwargs.get("time_name", "time"),
            "drop_vars": kwargs.get("drop_vars"),
            "engine": kwargs.get("engine"),
        }
        bundle = load_era5_land_dataset(_resolve_file_path(file_path), **era5_land_cfg)
        return bundle
    else:  # synthetic
